...
import (
	...
	"github.com/mati-fp/go-hexagonal/adapters/dto"
...
func createProduct(service application.ProductServiceInterface) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		err := json.NewDecoder(r.Body).Decode(&productDto) // pega o que ta vindo no body e joga para var dto
		if err != nil {
			w.WriteHeader(http.StatusInternalServerError)
		...
		err = json.NewEncoder(w).Encode(product) // joga o produto criado para o response
		if err != nil {
			w.WriteHeader(http.StatusInternalServerError)
			w.Write(jsonError(err.Error()))
			return
